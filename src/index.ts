...
const fit = (points: Point[], closed: unknown, error: number) => {
  ...
  // closed path.
  if (closed) {
    points.unshift(points[points.length - 1] as Point);
    points.push(points[1] as Point); // The point previously at index 0 is now 1.
  }
  ...
  // to one segment:
  const segments = [new Segment(points[0] as Point)];
  fitCubic(
    points,
    segments,
    error,
    ...
    length - 1,
    // Left Tangent
    points[1]!._subtract(points[0] as Point),
    // Right Tangent
    points[length - 2]!._subtract(points[length - 1] as Point)
  );
...
const fitCubic = (
  ...
  if (last - first === 1) {
    const pt1 = points[first] as Point,
      pt2 = points[last] as Point,
      dist = pt1._getDistance(pt2) / 3;
    addCurve(segments, [
      pt1,
      pt1._add(tan1._normalize(dist)),
      pt2._add(tan2._normalize(dist)),
      pt2,
    ]);
    return;
  }
  ...
  const tanCenter = points[split! - 1]!._subtract(points[split! + 1] as Point);
...
const addCurve = (segments: Segment[], curve: readonly Point[]) => {
  const prev = segments[segments.length - 1] as Segment;
  prev._handleOut = curve[1]!._subtract(curve[0] as Point);
  segments.push(
    new Segment(curve[3] as Point, curve[2]!._subtract(curve[3] as Point))
  );
};
...
const generateBezier = (
  ...
  for (let i = 0, l = last - first + 1; i < l; i++) {
    const u = uPrime[i] as number,
      ...
      tmp = (points[first + i] as Point)
        ._subtract(pt1._multiply(b0 + b1))
        ._subtract(pt2._multiply(b2 + b3));
    // @ts-ignore
    C[0][0] += a1._dot(a1);
    // @ts-ignore
    C[0][1] += a1._dot(a2);
  ...
  return [
    pt1,
    pt1._add(handle1 || tan1._normalize(alpha1)),
    pt2._add(handle2 || tan2._normalize(alpha2)),
    pt2,
  ];
};
...
const reparameterize = (
  ...
  for (let i = first; i <= last; i++) {
    // @ts-ignore
    u[i - first] = findRoot(curve, points[i] as Point, u[i - first]);
  }
...
const findRoot = (curve: readonly Point[], point: Point, u: number) => {
  ...
  for (let i = 0; i <= 2; i++) {
    curve1[i] = curve[i + 1]!._subtract(curve[i] as Point)._multiply(3);
  }
  // Generate control vertices for Q''
  for (let i = 0; i <= 1; i++) {
    curve2[i] = curve1[i + 1]!._subtract(curve1[i] as Point)._multiply(2);
  }
  // Compute Q(u), Q'(u) and Q''(u)
  const pt = evaluate(3, curve, u) as Point,
    pt1 = evaluate(2, curve1, u) as Point,
    pt2 = evaluate(1, curve2, u) as Point,
...
const evaluate = (degree: number, curve: readonly Point[], t: number) => {
  // Copy array
  const tmp = curve.slice() as Point[];
...
const findMaxError = (
  ...
  let index = Math.floor((last - first + 1) / 2),
    maxDist = 0;
  for (let i = first + 1; i < last; i++) {
    const P = evaluate(3, curve, u[(i - first) as number] as number) as Point;
    const v = P._subtract(points[i] as Point);
...
const getSegmentsPathData = (
  ...
  const round =
    precision < 16
      ? (n: number) => Math.round(n * precisionMultiplier) / precisionMultiplier
      : (n: number) => n;
  ...
  for (let i = 0; i < length; i++) addSegment(segments[i] as Segment);
  // Close path by drawing first segment again
  if (closed && length > 0) {
    addSegment(segments[0] as Segment, true);
    parts.push('z');
  }
  return parts.join('');
};
...
const simplifySvgPath = (
  ...
  return getSegmentsPathData(
    fit(
      points.map(
        typeof (points[0] as { readonly x: number }).x === 'number'
          ? (p: any) => new Point(p.x, p.y)
          : (p: any) => new Point(p[0], p[1])
      ),
      options.closed,
